...
Importing into another software makes sense if not all intended modelling actions such as applying fillets are possible in Replicad. Another reason could be that your object is only a part in a larger assembly which is created in the other software. The following image shows an example where the expensive 3D program is used only to apply some additional fillets that caused a "kernel error" in Replicad. The blue object was exported as a STEP file from Replicad, the green object has the additional fillets applied with the Siemens Solid Edge program. 

<img src="https://github.com/raydeleu/ReplicadManual/blob/main/images/slottedlever_render.jpg" width="800"> 
...
As mentioned in the previous section, some of the programs allow to export the shape in a polygonal format and to determine the accuracy of these models. The following two images show the export of an object in OBJ and STL format using Moi3D. 
