...
function main()
{
    ...
    .ellipse(10,10,10,10,270,[0,0,1],false)
    .lineTo([0,10])
    .close()
    .extrude(1)
    .translate(0,-30,0)
...
A special way to create ars in your sketch is the rounding or fillet in sharp corners of your sketch. The method to achieve this is called `customCorner(radius)`, which uses the radius of the rounding as an argument. The method should be placed between the two methods used to define the sharp corner. The following code snippet shows an example how to create a rounded shape. Note that in this case, using a rounding radius that is exactly half the height of the shape fails. If you want to achieve a semi-circle at each end you have to use the arc methods described in the previous section. 
...
The boolean operations `cut`, `fuse` and `intersect` provide options to shortcut the creation of complicated drawings without the need for complex geometric calculations. This will be shown in the next section. 
