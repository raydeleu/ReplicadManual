...
The downside of the Visualiser is that the user has to edit a file using a text editor and then import this file into the Visualiser. In the so-called *Workbench*, available at https://studio.replicad.xyz/workbench a code editor is included. The code editor pane on the left can be used to write the code, while the visualiser window on the right directly shows the result of changes in the editor window. 
