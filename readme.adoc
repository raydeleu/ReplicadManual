...
It is not always necessary to use the configuration at the begin and end point of a smoothSpline. In the example in <<mouse>> the `.smoothSpline` method is used between two arcs. Perhaps it is just luck, but here no additional configuration was necessary. 
...
---
._Using a thin walled shape with an offset_ 
