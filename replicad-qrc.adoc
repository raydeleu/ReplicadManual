...
m| .extrude(distance,extrusionConfig?)  |
m|             extrusionConfig          | {     extrusionDirection:[point],
                                         ExtrusionProfile:ExtrusionProfile,
                                        origin:[point],
                                        twistAngle:deg}

m|             extrusionProfile:        |   {     profile:"linear" / "s-curve",
                                            endFactor: scale}

m| .loftWith([otherSketches],loftConfig,returnShell?) |

            loftConfig =        |       {   endPoint:[point],
                                        ruled: boolean,
                                    startPoint:[point]}

m| .revolve(revolutionAxis:[point],config?)    | default is z-axis

m|            config                            |      origin:[point]
...
m|             sketchRectangle(2, 30, { plane, origin })        |
...
[cols="1,3",stripes=even]
|===
m| .chamfer(radiusConfig,filter?)
m| .fillet(radiusConfig,filter?)
m| .shell(thickness, (f) => f.inPlane("YZ",-20),{tolerance:number})
m|                     radiusConfig    = number or func
m|                     filter          = (e) => e.Edgefinder
m| makeOffset(shape,thickness)
m| addHolesInFace(face,holeWires[])
|===
...
m| find(shape,options)         // returns all the elements that fit the filters
            options {unique: true}
...
[cols="1,3",stripes=even]
|===
m| .inDirection([x,y,z]|"X"|"Y"|"Z")
m| .ofLength(number)
m| .ofCurveType(  todo?)
m| .parallelTo(plane | StandardPlane e.g. "XY")
m| .inPlane(PlaneName | Plane)
m| .shouldKeep todo? 
...
[cols="1,3",stripes=even]
|===
m| .cut(tool,{optimisation:"none" | "commonFace" | "sameFace"})
m| .fuse(otherShape,.. )
m| .intersect(tool)
