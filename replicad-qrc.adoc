...
== Visualiser
A model in Replicad is built using a javascript input file (see next Section). This file defines a "main" function that should return the shapes that you want to visualise or export. The easiest way to check that the part is built correctly and to export the result is to use the *Visualiser* that is offered on the Replicad site https://replicad.xyz. 
